...
func PostMovimientoCaja(req *core.HandlerArgs) core.HandlerResponse {
    ...
    if err := scylla.Insert(&[]s.CajaMovimiento{record}); err != nil {
        return req.MakeErr("Error al registrar movimiento.", err)
    }

    q1 := scylla.Table[s.Caja]()
    if err := scylla.Update(&[]s.Caja{caja}, q1.SaldoCurrent, q1.Updated); err != nil {
...
The ORM auto-assigns IDs in-place during `scylla.Insert` for any record whose ID is `<= 0`, using the schema's `Autoincrement(N)` configuration. The handler does **not** call `GetCounter` directly — it captures each record's `TempID` first, calls `scylla.Insert(&records)`, then reads back the assigned `records[i].ID` to build the mapping.
...
func PostBatch(req *core.HandlerArgs) core.HandlerResponse {
    ...
    // 1. Capture client TempIDs before persistence overwrites them.
    ...
    if err := scylla.Insert(&records); err != nil {
        return req.MakeErr("Error al guardar el lote.", err)
    }

    // 3. Pair each TempID with the server-assigned ID.
    for i := range records {
...
func PostCajas(req *core.HandlerArgs) core.HandlerResponse {
	...
	if body.ID <= 0 {
		...
		q1 := scylla.Table[s.Caja]()
		err = scylla.UpdateExclude(&cajaBatch, q1.CuadreFecha, q1.CuadreSaldo, q1.SaldoCurrent)
	}
	...
	// Pick up the server-assigned ID (if newly inserted) for the response.
	return req.MakeResponse(cajaBatch[0])
}
```
...
6.  **Protect immutable fields**: Use `scylla.UpdateExclude` for fields like `Created`, `CreatedBy`, or calculated balances during a standard update.
...
- [ ] New records (`ID <= 0`) rely on the ORM to auto-assign the ID during `scylla.Insert` via the schema's `Autoincrement(N)`. Read back `records[i].ID` after insert when you need the assigned values.
- [ ] Sensitive fields are protected using `scylla.UpdateExclude`.
