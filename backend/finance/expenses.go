...
func PostExpenses(req *core.HandlerArgs) core.HandlerResponse {
	...
	body.Type = core.If(body.Type == 0, financeTypes.ExpenseTypeSimple, body.Type)
	if body.Type != financeTypes.ExpenseTypeSimple && body.Type != financeTypes.ExpenseTypeInventory {
		return req.MakeErr("Tipo de gasto inválido: sólo se puede registrar Simple o Inventario.")
	}
	if body.Type == financeTypes.ExpenseTypeInventory && body.ProductID <= 0 {
	...
	if body.ID <= 0 {
		...
		if existing[0].Status == financeTypes.ExpenseStatusPosted {
...
func PostExpensePayment(req *core.HandlerArgs) core.HandlerResponse {
	...
	if expense.Status == financeTypes.ExpenseStatusPosted || expense.Type == financeTypes.ExpenseTypeDepreciation {
