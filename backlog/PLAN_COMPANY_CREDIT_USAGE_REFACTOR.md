...
`fareward/src/limiter/storage.rs` is **not** touched: `db.TypeView` compiles to a real Scylla
...
2. ~~**Materialized view cost.**~~ Resolved: the limiter flushes dirty rows only, every 15 s
   (`fareward/src/limiter/mod.rs:9`), so an active `(company, user, day)` row costs 4 writes per
...
Touched beyond §8: `fareward` (`aggregation.rs` owns the sentinel, `storage.rs` picks the table
...
- Rust (`fareward/src/limiter/time_frame.rs`) bucketed `daily()` as `unix_seconds / 86_400` —
  a UTC day.
