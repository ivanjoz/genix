...
// One row every five seconds with what the machine was doing, written by fareward and never by
// this backend: the ORM owns the schema, the daemon owns the writes. That is the same split
// user_logs uses, and here it is not a preference — fareward is the only Genix process
...
type ServerMetric struct {
	...
	// These carry no column tag, so the ORM derives fareward_mem_mb / fareward_cpu_percent from
	// the field name — the exact column names the daemon's INSERT lists, and the JSON keys the
	// Server Panel reads. They were server_utils_* until the fareward rename, which makes this a
	// schema change and not only a rename: the ORM adds missing columns and never drops them, so a
	// deployed table gains this pair and keeps the old one until its rows expire under the TTL.
	// Nothing back-fills, so windows sampled before the deploy read as not-measured here.
	FarewardMemMb      int16 `json:",omitempty"`
	FarewardCpuPercent int16 `json:",omitempty"`
...
type ServerMetricTable struct {
	...
	// See the record above: renaming these two is a Scylla schema change, not just a rename.
	FarewardMemMb      db.Col[*ServerMetricTable, int16]
	FarewardCpuPercent db.Col[*ServerMetricTable, int16]
...
// config.toml, which fareward validates against this same arithmetic: change one and the
