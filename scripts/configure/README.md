...
only then invokes the nested installer. A downloaded latest asset takes precedence over an older
installed binary. Option `7` requires the corresponding source tree and never falls back to a
binary.
