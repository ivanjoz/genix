...
func GetSupplyMaterials(req *core.HandlerArgs) core.HandlerResponse {
	...
	supplyMaterialRecords := []logisticaTypes.SupplyMaterial{}
	supplyMaterialQuery := db.Query(&supplyMaterialRecords)
	supplyMaterialQuery.Select().CompanyID.Equals(req.Usuario.EmpresaID)
...
func PostSupplyMaterial(req *core.HandlerArgs) core.HandlerResponse {
	incomingSupplyMaterials := []logisticaTypes.SupplyMaterial{}
	...
	toInsert := []logisticaTypes.SupplyMaterial{}
	toUpdate := []logisticaTypes.SupplyMaterial{}
	for recordIndex := range incomingSupplyMaterials {
		...
		supplyMaterialRecord.CompanyID = req.Usuario.EmpresaID
		supplyMaterialRecord.Updated = currentTimestamp
		supplyMaterialRecord.UpdatedBy = req.Usuario.ID
		...
		if supplyMaterialRecord.ID <= 0 {
			...
			supplyMaterialRecord.CreatedBy = req.Usuario.ID
	...
	if len(toUpdate) > 0 {
		supplyMaterialTable := db.Table[logisticaTypes.SupplyMaterial]()
