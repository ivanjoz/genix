...
func PostGaleriaImage(req *core.HandlerArgs) core.HandlerResponse {
	...
	galeriaImage := businessTypes.GalleryImage{
	...
	err = db.Insert(&[]businessTypes.GalleryImage{galeriaImage})
	if err != nil {
...
func GetGaleriaImages(req *core.HandlerArgs) core.HandlerResponse {
	...
	imagenes := []businessTypes.GalleryImage{}
	query := db.Query(&imagenes)
	q1 := db.Table[businessTypes.GalleryImage]()

	query.Exclude(q1.CompanyID).
