...
// is with its session token, and both this process and the bridge check that
// the identity inside the token matches the authenticated one (see
// PostAgentTurn) — otherwise editing a token would be a way to address another
// tenant's tab.
//
// Inside this package the routing key stays the *tab id*: company and user
...
//
// Mirrored in auth-limiter/src/bridge/token.rs and frontend/core/agent/channel.ts. The
