...
func GetListasCompartidas(req *core.HandlerArgs) core.HandlerResponse {
	...
	listaRegistrosMap := map[int32]*[]negocioTypes.ListaCompartidaRegistro{}
	for _, listaID := range listasIDs {
		listaRegistrosMap[listaID] = &[]negocioTypes.ListaCompartidaRegistro{}
	}
	eg := errgroup.Group{}
	...
	response := map[string]*[]negocioTypes.ListaCompartidaRegistro{}
...
func PostListasCompartidas(req *core.HandlerArgs) core.HandlerResponse {
	...
	for index := range records {
		...
		newIDs = append(newIDs, negocioTypes.NewIDToID{TempID: e.ID})
	...
	existingRecordsByHash := []negocioTypes.ListaCompartidaRegistro{}
	...
	existingRecordsGroupedByRecordKey := core.SliceToMapP(existingRecordsByHash,
		func(e negocioTypes.ListaCompartidaRegistro) string {
