...
func isSupportedPackedIndexNumericFieldType(fieldType string) bool {
	// Packed indexes only support scalar integers. Slices are explicitly rejected elsewhere.
	switch fieldType {
...
func makeTable[T TableSchemaInterface[T]](structType *T) ScyllaTable[any] {
	...
	for _, indexColumns := range schema.Indexes {
		// TableSchema.Indexes: build a stored packed numeric column plus a local index on (partition, packed).
		// Rationale: this lets us answer queries like "Status IN (...) + Updated BETWEEN/GT" without ALLOW FILTERING.
		if len(indexColumns) < 2 {
			panic(fmt.Sprintf(`Table "%v": Indexes entries must have at least 2 columns. Found: %v`, dbTable.name, len(indexColumns)))
		}
		...
		// Build from schema column configs so we can read DecimalSize() and Int32() flags.
		for columnIndex, indexColumnConfig := range indexColumns {
			...
			if column == nil {
				panic(fmt.Sprintf(`Table "%v": Indexes column "%v" was not found`, dbTable.name, configInfo.Name))
			}

			if column.GetType().IsComplexType || column.GetType().IsSlice || !isSupportedPackedIndexNumericFieldType(column.GetType().FieldType) {
				panic(fmt.Sprintf(`Table "%v": Indexes column "%v" must be a scalar integer. Found: %v`, dbTable.name, column.GetName(), column.GetType().FieldType))
			}
			...
			// This keeps the schema declaration explicit and avoids accidental overflow risk.
			if columnIndex == 0 && configInfo.decimalSize > 0 {
				panic(fmt.Sprintf(`Table "%v": Indexes first column "%v" must not set DecimalSize()`, dbTable.name, configInfo.Name))
			}
			if columnIndex > 0 && configInfo.decimalSize <= 0 {
				panic(fmt.Sprintf(`Table "%v": Indexes requires DecimalSize() for column "%v" (all columns after the first must set DecimalSize)`, dbTable.name, configInfo.Name))
			}
		...
		// Compute digit budget and resolve the first slot digits (remaining space).
		sumTrailingDigits := int64(0)
		for i := 1; i < len(slotDigitsPerColumn); i++ {
			sumTrailingDigits += slotDigitsPerColumn[i]
		}

		// Enforce output width constraints (leave 1 digit for the first component).
		if isInt32Packed {
			if sumTrailingDigits > 8 {
				panic(fmt.Sprintf(`Table "%v": int32 packed Indexes requires sum(DecimalSize(columns[1:])) <= 8. Got: %v`, dbTable.name, sumTrailingDigits))
			}
		} else {
			if sumTrailingDigits > 18 {
				panic(fmt.Sprintf(`Table "%v": int64 packed Indexes requires sum(DecimalSize(columns[1:])) <= 18. Got: %v`, dbTable.name, sumTrailingDigits))
			}
		}

		firstSlotDigits := totalDigits - sumTrailingDigits
		if firstSlotDigits <= 0 {
			panic(fmt.Sprintf(`Table "%v": Indexes invalid digit budget: totalDigits=%v sumTrailingDigits=%v`, dbTable.name, totalDigits, sumTrailingDigits))
		}
		slotDigitsPerColumn[0] = firstSlotDigits

		virtualPackedColName := fmt.Sprintf("zz_ixp_%s", strings.Join(sourceColumnNames, "_"))
		if _, exists := dbTable.columnsMap[virtualPackedColName]; exists {
			panic(fmt.Sprintf(`Table "%v": generated packed index column already exists: %v`, dbTable.name, virtualPackedColName))
		}

		// Always compute the packed value as int64, then optionally trim/cast to int32 at the very end.
		packedColumnTypeName := "int64"
		if isInt32Packed {
			packedColumnTypeName = "int32"
		}

		// Capture immutable locals for closures.
		...
		virtualPackedColumn.getRawValue = func(ptr unsafe.Pointer) any {
			...
			for _, sourceColumn := range sourceColumnsLocal {
				raw := sourceColumn.GetRawValue(ptr)
				valueI64 := convertToInt64(raw)
				if valueI64 < 0 {
					panic(fmt.Sprintf(`Table "%v": packed Indexes column "%v" produced negative value %d`, dbTable.name, sourceColumn.GetName(), valueI64))
				}
			...
			// Keep the stored value within int32 constraints by trimming on the right side (least-significant digits).
			// This may overfetch on reads, so query execution must post-filter for exact semantics.
		...
		// Register the packed local index on ((partition), packedCol).
		partitionColumn := dbTable.GetPartKey()
		if partitionColumn == nil || partitionColumn.IsNil() {
			panic(fmt.Sprintf(`Table "%v": Indexes requires a Partition column`, dbTable.name))
		}

		indexName := fmt.Sprintf(`%v__%v_index_1`, dbTable.name, virtualPackedColName)
		packedIndex := &viewInfo{
			Type:               2,
			name:               indexName,
			idx:                idxCount,
			column:             virtualPackedColumn,
			columns:            slices.Clone(sourceColumnNames), // used for matching + update dependency checks
			RequiresPostFilter: true,                            // DecimalSize() truncation can overfetch; filter must enforce exactness
		}
		packedIndex.getCreateScript = func() string {
			return fmt.Sprintf(`CREATE INDEX %v ON %v ((%v),%v)`,
				indexName, dbTable.GetFullName(), partitionColumn.GetName(), virtualPackedColName)
		}

		// WHERE rewriting: map (source col predicates) => (packed col range/equality predicates).
		...
		packedIndex.getStatement = func(statements ...ColumnStatement) []string {
			...
			// Build cartesian groups for all prefix columns (all but the last).
			...
			for _, prefixValues := range prefixValueGroups {
				...
				case ">", ">=", "<", "<=":
					boundValue := convertToInt64(lastStatement.Value)
					whereStatements = append(whereStatements, emitRangeClause(prefixValues, lastStatement.Operator, boundValue))
				default:
					return nil
				}
			}
		...
		// Register packed index metadata for capability computation.
