...
The ORM auto-assigns IDs in-place during `db.Insert` for any record whose ID is `<= 0`, using the schema's `Autoincrement(N)` configuration. The handler does **not** call `GetCounter` directly — it captures each record's `TempID` first, calls `db.Insert(&records)`, then reads back the assigned `records[i].ID` to build the mapping.
...
func PostCajas(req *core.HandlerArgs) core.HandlerResponse {
	...
	if body.ID <= 0 {
		...
		q1 := db.Table[s.Caja]()
		err = db.UpdateExclude(&cajaBatch, q1.CuadreFecha, q1.CuadreSaldo, q1.SaldoCurrent)
	}
