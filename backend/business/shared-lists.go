...
func GetSharedLists(req *core.HandlerArgs) core.HandlerResponse {
...
// PostSharedLists performs batch upsert by (ListID + normalized name hash).
// Behavior summary:
...
func PostSharedLists(req *core.HandlerArgs) core.HandlerResponse {
	...
	for index := range records {
		...
		incomingRecord.CompanyID = req.User.CompanyID
		...
		if incomingRecord.ID <= 0 && reusableDeletedRecordID > 0 {
			core.Log("PostSharedLists:: reutilizando ID por NameHash", incomingRecord.ListID, incomingRecord.NameHash, reusableDeletedRecordID)
	...
	for _, listID := range uniqueIncomingListIDs {
		...
		if cacheErr := core.SaveCacheGlobal(cacheGroup, req.User.CompanyID, nil, nowTime); cacheErr != nil {
			core.Log("PostSharedLists:: error registrando cambio de lista para ecommerce", cacheErr)
		}
	}
