...
// it belongs on the balance sheet. Mirrors backend/finance/types/expenses.go.
//
// Buying a fixed asset is deliberately absent: an asset is not an expense and writes no row
// here. Its cost and payment state live on the Asset, in the Activos page. The only thing an
// asset ever contributes to this table is its monthly depreciation.
export const ExpenseType = {
  SIMPLE: 1,       // Straight to P&L.
  INVENTORY: 2,    // Cash → stock.
  DEPRECIATION: 4, // Non-cash, generated by the server. Never appears in the payment tabs.
}

export const expenseTypeLabels: Record<number, string> = {
  [ExpenseType.SIMPLE]: "Simple|Simple",
  [ExpenseType.INVENTORY]: "Inventory|Inventario",
...
export const postInventoryExpense = (data: IInventoryExpense): Promise<IExpense> => {
  return POST({
    data,
    route: "expense-inventory",
    refreshRoutes: ["expenses", "supply-material"],
  })
}
