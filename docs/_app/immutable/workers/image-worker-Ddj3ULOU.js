...
async function init(module, moduleOptionOverrides) {
	...
	if (!isRunningInNode() && !isRunningInCloudflareWorker() && await threads()) {
		emscriptenModule = initEmscriptenModule((await import("./chunks/C7Ht7JMD.js")).default, actualModule, actualOptions);
		return emscriptenModule;
	}
	emscriptenModule = initEmscriptenModule((await import("./chunks/CNwNN7-h.js")).default, actualModule, actualOptions);
	return emscriptenModule;
}
...
self.onmessage = async (event) => {
	...
	try {
		...
		if (inputBitmap) {
			...
			const blob$1 = inputBlob;
			console.log("🔄 Creating ImageBitmap from blob...");
			bitmap = await createImageBitmap(blob$1);
