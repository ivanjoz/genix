...
func GetSystemMemoryPackages(req *core.HandlerArgs) core.HandlerResponse {
	...
	core.Log("System memory packages report solicitado | user:", req.User.ID, "| route:", req.Route, "| limit:", requestedLimit)
	...
	core.Log("System memory packages report generado | user:", req.User.ID, "| packages:", len(goHeapPackageReport.TopPackages), "| warnings:", len(reportWarnings), "| go_heap_inuse:", goHeapPackageReport.GoHeapInuseBytes)
