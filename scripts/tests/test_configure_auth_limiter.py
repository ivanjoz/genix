...
def load_configure_auth_limiter_module():
    ...
    script_path = CONFIGURE_DIRECTORY / "configure_auth_limiter.py"
    module_spec = importlib.util.spec_from_file_location("configure_auth_limiter", script_path)
    configure_auth_limiter = importlib.util.module_from_spec(module_spec)
    module_spec.loader.exec_module(configure_auth_limiter)
    return configure_auth_limiter


class BridgeNginxTemplateTest(unittest.TestCase):
    def test_certificate_produces_an_http3_vhost_that_never_buffers(self):
        configure_auth_limiter = load_configure_auth_limiter_module()
        existing_config = (
            "server {\n"
        ...
        with redirect_stdout(io.StringIO()):
            rendered_config = configure_auth_limiter.build_bridge_nginx_configuration(
    ...
    def test_reuseport_owner_is_found_in_any_nginx_configuration_file(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            configure_auth_limiter.NGINX_CONFIGURATION_DIRECTORY = configuration_directory

            own_configuration_path = configuration_directory / "genix-sse.un.pe.conf"
            self.assertIsNone(configure_auth_limiter.detect_reuseport_listener_owner(own_configuration_path))
            ...
            self.assertEqual(
                configure_auth_limiter.detect_reuseport_listener_owner(own_configuration_path),
...
class ConfigurationResolutionTest(unittest.TestCase):
    def test_domain_is_taken_from_the_url_in_any_accepted_shape(self):
        ...
        with redirect_stdout(io.StringIO()):
            for configured_url in ("https://genix-sse.un.pe/", "http://Genix-SSE.un.pe", "genix-sse.un.pe"):
                self.assertEqual(
                    configure_auth_limiter.resolve_bridge_domain(
    ...
    def test_an_unusable_url_stops_the_run_instead_of_prompting(self):
        ...
        for project_credentials in unusable_credentials:
            with redirect_stdout(io.StringIO()), self.assertRaises(SystemExit):
                configure_auth_limiter.resolve_bridge_domain(project_credentials, config_path)

    def test_port_defaults_to_the_value_compiled_into_the_binary(self):
        configure_auth_limiter = load_configure_auth_limiter_module()

        with redirect_stdout(io.StringIO()):
            self.assertEqual(configure_auth_limiter.resolve_bridge_port({}), 14012)
            self.assertEqual(
                configure_auth_limiter.resolve_bridge_port({"sse_bridge": {"port": "15000"}}), 15000
            )

            # A typo must stop the run: the Nginx upstream is built from this number.
            with self.assertRaises(SystemExit):
                configure_auth_limiter.resolve_bridge_port({"sse_bridge": {"port": "0"}})
    ...
    def test_service_only_mode_does_not_require_a_bridge_domain_or_nginx(self):
        ...
        with mock.patch.object(
            configure_auth_limiter, "parse_command_arguments", return_value=command_arguments
        ), mock.patch.object(configure_auth_limiter, "require_root_execution"), mock.patch.object(
            configure_auth_limiter,
            "detect_repository_config_path",
            return_value=Path("/repo/config.toml"),
        ), mock.patch.object(
            configure_auth_limiter, "load_project_credentials", return_value={}
        ), mock.patch.object(configure_auth_limiter, "verify_required_settings"), mock.patch.object(
            configure_auth_limiter, "ensure_rate_limit_credit_limits"
        ), mock.patch.object(
            configure_auth_limiter, "resolve_bridge_domain"
        ) as resolve_domain_mock, mock.patch.object(
            configure_auth_limiter, "resolve_bridge_port", return_value=14012
        ), mock.patch.object(
            configure_auth_limiter, "resolve_listen_port", return_value=14013
        ), mock.patch.object(
            configure_auth_limiter, "resolve_listen_is_public", return_value=False
        ), mock.patch.object(
            configure_auth_limiter, "resolve_client_address", return_value="127.0.0.1:14013"
        ), mock.patch.object(
            configure_auth_limiter, "install_systemd_service", return_value="homelab"
        ), mock.patch.object(
            configure_auth_limiter, "configure_bridge_nginx_vhost"
        ) as configure_nginx_mock, mock.patch.object(
            configure_auth_limiter, "open_listen_port_in_firewall"
        ), mock.patch.object(
            configure_auth_limiter, "print_summary"
        ) as print_summary_mock:
            configure_auth_limiter.main()
    ...
    def test_both_secrets_must_be_present_and_are_never_prompted_for(self):
        ...
        with redirect_stdout(io.StringIO()):
            ...
            configure_auth_limiter.verify_required_settings(
    ...
    def test_the_failure_report_reads_the_real_cause_from_the_journal(self):
        ...
        bind_failure_journal = (
            "auth-limiter[58290]: INFO scylla::cluster::worker: Node added to cluster\n"
            "auth-limiter[58290]: Error: failed to bind 150.136.42.240:14013\n"
            "auth-limiter[58290]:     Cannot assign requested address (os error 99)\n"
            "systemd[1]: auth-limiter.service: Failed with result 'exit-code'.\n"
        )
        self.assertIn(
            "[auth_limiter] public",
            configure_auth_limiter.diagnose_service_failure(bind_failure_journal),
        )

        # The undeployed-tables hypothesis is still reported — when the journal actually says so.
        self.assertIn(
            "check_tables",
            configure_auth_limiter.diagnose_service_failure(
                "auth-limiter[1]: Error: unconfigured table credit_usage_company\n"
            ),
        )

        # An unrecognized failure returns nothing, so the caller can say so instead of guessing.
        self.assertIsNone(configure_auth_limiter.diagnose_service_failure("nothing familiar here"))
    ...
    def test_a_missing_optional_binary_does_not_crash_the_run(self):
        ...
        with redirect_stdout(io.StringIO()):
            result = configure_auth_limiter.run_command(
                ["genix-binary-that-does-not-exist"], allow_failure=True
            )
        self.assertEqual(result.returncode, 127)

        with redirect_stdout(io.StringIO()), self.assertRaises(SystemExit):
            configure_auth_limiter.run_command(["genix-binary-that-does-not-exist"])
    ...
    def test_the_script_never_imports_an_interactive_prompt(self):
        ...
        script_source = (CONFIGURE_DIRECTORY / "configure_auth_limiter.py").read_text(encoding="utf-8")
...
class UnitAndBinaryTest(unittest.TestCase):
    def test_service_unit_reads_both_secrets_from_the_config_file(self):
        ...
        unit_contents = configure_auth_limiter.build_service_contents(
        ...
        self.assertIn("ExecStart=/usr/local/bin/genix/auth-limiter", unit_contents)
    ...
    def test_the_c_linker_is_installed_only_when_it_is_missing(self):
        ...
        with mock.patch.object(configure_auth_limiter.shutil, "which", lambda name: "/usr/bin/cc"):
            with mock.patch.object(configure_auth_limiter, "run_command", fake_run_command):
                with redirect_stdout(io.StringIO()):
                    configure_auth_limiter.ensure_c_linker()
        ...
        with mock.patch.object(
            configure_auth_limiter.shutil, "which", lambda name: available_tools.get(name)
        ):
            with mock.patch.object(configure_auth_limiter, "run_command", fake_run_command):
                with redirect_stdout(io.StringIO()):
                    with self.assertRaises(SystemExit):
    ...
    def test_service_unit_raises_the_file_descriptor_limit(self):
        ...
        service_contents = configure_auth_limiter.build_service_contents(
    ...
    def test_restart_watcher_units_target_the_merged_service(self):
        ...
        self.assertIn(
            "PathChanged=/usr/local/bin/genix/auth-limiter",
            configure_auth_limiter.build_restart_path_contents(),
        )
        self.assertIn(
            "ExecStart=/usr/bin/systemctl restart auth-limiter.service",
            configure_auth_limiter.build_restart_service_contents(),
        )
    ...
    def test_source_is_detected_only_with_a_cargo_manifest_and_main(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory, redirect_stdout(io.StringIO()):
            ...
            self.assertIsNone(configure_auth_limiter.detect_source_directory(repository_root))

            source_directory = repository_root / "auth-limiter"
            ...
            self.assertEqual(
                configure_auth_limiter.detect_source_directory(repository_root), source_directory
            )
    ...
    def test_prebuilt_binary_is_staged_and_renamed_into_place(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory, redirect_stdout(io.StringIO()):
            repository_root = Path(temporary_directory)
            configure_auth_limiter.SERVICE_INSTALL_DIRECTORY = repository_root / "installed"
            configure_auth_limiter.BINARY_PATH = (
                configure_auth_limiter.SERVICE_INSTALL_DIRECTORY / "auth-limiter"
            )
            configure_auth_limiter.SERVICE_INSTALL_DIRECTORY.mkdir()

            # No source to compile and no artifact anywhere: that must stop the run.
            with self.assertRaises(SystemExit):
                configure_auth_limiter.provide_binary(repository_root, runtime_user_entry)

            artifact_path = (
                repository_root
                / "tmp"
                / f"auth-limiter_linux_{configure_auth_limiter.resolve_go_architecture()}"
            )
            ...
            self.assertTrue(configure_auth_limiter.BINARY_PATH.is_file())
            self.assertTrue(os.access(configure_auth_limiter.BINARY_PATH, os.X_OK))
            self.assertFalse(
                (configure_auth_limiter.SERVICE_INSTALL_DIRECTORY / ".auth-limiter.staged").exists()
            )
    ...
    def test_precompiled_mode_ignores_present_rust_source(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory, redirect_stdout(io.StringIO()):
            ...
            with mock.patch.object(configure_auth_limiter, "compile_binary") as compile_mock:
                configure_auth_limiter.provide_binary(
                    repository_root,
                    runtime_user_entry,
                    configure_auth_limiter.BINARY_SOURCE_PRECOMPILED,
                )

            compile_mock.assert_not_called()
            self.assertEqual(configure_auth_limiter.BINARY_PATH.read_bytes(), artifact_path.read_bytes())
    ...
    def test_cargo_is_looked_up_where_sudo_cannot_see_it(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                self.assertEqual(configure_auth_limiter.detect_cargo_binary(None), fake_cargo)
            finally:
        ...
        try:
            with redirect_stdout(io.StringIO()), self.assertRaises(SystemExit):
                configure_auth_limiter.detect_cargo_binary(None)
        finally:
            del os.environ["CARGO_BINARY"]
...
class ListenPortFirewallTest(unittest.TestCase):
    ...
    def test_the_listen_port_falls_back_to_the_rust_default_and_rejects_a_typo(self):
        ...
        with redirect_stdout(io.StringIO()):
            self.assertEqual(configure_auth_limiter.resolve_listen_port({}), 14013)
            self.assertEqual(
                configure_auth_limiter.resolve_listen_port({"auth_limiter": {"port": 14099}}),
                14099,
            )

        # Never guess: this number decides which port gets a firewall rule.
        with redirect_stdout(io.StringIO()), self.assertRaises(SystemExit):
            configure_auth_limiter.resolve_listen_port({"auth_limiter": {"port": "catorce mil"}})

    def test_public_is_read_the_same_way_the_daemon_reads_it(self):
        configure_auth_limiter = load_configure_auth_limiter_module()
        resolve = configure_auth_limiter.resolve_listen_is_public

        # Absent means loopback, matching optional_bool in auth-limiter/src/config.rs.
        self.assertFalse(resolve({}))
        self.assertFalse(resolve({"auth_limiter": {"public": False}}))
        self.assertTrue(resolve({"auth_limiter": {"public": True}}))
        # The daemon accepts a quoted boolean, so this script must not call it a typo.
        self.assertTrue(resolve({"auth_limiter": {"public": "true"}}))

        with redirect_stdout(io.StringIO()), self.assertRaises(SystemExit):
            resolve({"auth_limiter": {"public": "yes please"}})
    ...
    def test_a_private_daemon_never_touches_the_firewall(self):
        ...
        with mock.patch.object(configure_auth_limiter, "ensure_tcp_port_open") as open_port:
            with redirect_stdout(io.StringIO()) as captured_output:
                configure_auth_limiter.open_listen_port_in_firewall(14013, False)
    ...
    def test_a_public_daemon_opens_the_port_and_still_points_at_the_cloud_ingress_rules(self):
        ...
        with mock.patch.object(
            configure_auth_limiter, "ensure_tcp_port_open", return_value=True
        ) as open_port:
            with redirect_stdout(io.StringIO()) as captured_output:
                configure_auth_limiter.open_listen_port_in_firewall(14013, True)

        open_port.assert_called_once_with(14013, "auth_limiter raw TCP")
    ...
    def test_a_firewall_that_could_not_be_opened_warns_instead_of_aborting(self):
        ...
        with mock.patch.object(configure_auth_limiter, "ensure_tcp_port_open", return_value=False):
            with redirect_stdout(io.StringIO()) as captured_output:
                configure_auth_limiter.open_listen_port_in_firewall(14013, True)
    ...
    def test_the_client_address_is_derived_the_way_the_backend_derives_it(self):
        configure_auth_limiter = load_configure_auth_limiter_module()
        resolve = configure_auth_limiter.resolve_client_address

        # A private daemon is loopback-only whatever host is written, exactly like
        # makeAuthLimiterAddress in backend/core/security.go.
        credentials = {"auth_limiter": {"host": "150.136.42.240"}}
